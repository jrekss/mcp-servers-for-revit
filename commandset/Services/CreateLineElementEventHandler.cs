...
namespace RevitMCPCommandSet.Services
{
    public class CreateLineElementEventHandler : IExternalEventHandler, IWaitableExternalEventHandler
    {
        ...
        public void Execute(UIApplication uiapp)
        {
            ...
            try
            {
                ...
                foreach (var data in CreatedInfo)
                {
                    ...
                    if (data.TypeId != -1 && data.TypeId != 0)
                    {
                        ...
                        if (typeELeId != null)
                        {
                            ...
                            if (typeEle != null && typeEle is FamilySymbol)
                            {
                                ...
                                // 获取symbol的Categoryobject and convert toBuiltInCategory枚举
                    ...
                    using (Transaction transaction = new Transaction(doc, "Create point-based component"))
                    {
                        transaction.Start();
                        switch (builtInCategory)
                        {
                            ...
                            default:
                                ...
                                // 调用FamilyInstanceGeneric creation method
            ...
            catch (Exception ex)
            {
                ...
                {
                    Success = false,
                    Message = $"Error creating linear component: {ex.Message}",
                };
                TaskDialog.Show("错误", $"Error creating linear component: {ex.Message}");
            }
            finally
            {
        ...
        private WallType CreateOrGetWallType(Document doc, double width = 200 / 304.8)
        {
            ...
            // First search for the existence of a basic wall type with the specified thickness.
            ...
            // If it does not exist, create a new wall type based on the basic wall.
            ...
            if (baseWallType == null)
                throw new InvalidOperationException("No available basic wall type found");
            ...
            if (cs != null)
            {
                ...
                CompoundStructureLayer newLayer = new CompoundStructureLayer(
                    ...
                    MaterialFunctionAssignment.Structure,  // Function allocation
                    materialId  // 材料ID
                );
        ...
        private DuctType CreateOrGetDuctType(Document doc, double width, double height)
        {
            ...
            // First search for the existence of a duct type with the specified dimensions.
            ...
            // If it does not exist, create a new duct type based on the existing rectangular duct type.
            ...
            if (baseDuctType == null)
                throw new InvalidOperationException("No available basic rectangular duct type found.");

            // Duplicate duct type
            DuctType newDuctType = baseDuctType.Duplicate(typeName) as DuctType;

            // Set duct size parameters
